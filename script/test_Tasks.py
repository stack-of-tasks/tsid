...
Kp = 1 * np.matrix(np.ones(3)).transpose()
Kd = 2.0 * np.matrix(np.ones(3)).transpose()
taskAM.setKp(Kp)
taskAM.setKd(Kd)
...
am_ref  =np.matrix([0, 0, 0.02]).transpose()
traj = tsid.TrajectoryEuclidianConstant("traj_am", am_ref)
sample = tsid.TrajectorySample(0)
...
for i in range(0, max_it):
    ...
    if error < 1e-8:
        ...
        break
    if i%100 == 0:
        print "Time :", t, "Momentum error :", error, "Current Momentum :", np.linalg.norm(taskAM.momentum) 
