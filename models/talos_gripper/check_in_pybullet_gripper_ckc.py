...
p.resetDebugVisualizerCamera( cameraDistance=.2, cameraYaw=115, cameraPitch=-25, cameraTargetPosition=[0,0,0.05])
humanoid = p.loadURDF("urdf/talos_gripper_half.urdf",[0,0,0],[1,0,0,0], useFixedBase=True)
...
for j in range (p.getNumJoints(humanoid)):
	...
	if (jointType==p.JOINT_PRISMATIC or jointType==p.JOINT_REVOLUTE):
		...
		paramIds.append(p.addUserDebugParameter(jointName.decode("utf-8"),-4,4,refAngle))
...
c1 = p.createConstraint( humanoid, 
...
while(1):
	...
	for i in range(len(paramIds)):
		...
		if jointIds[i] in passiveJointIds:
			...
			p.setJointMotorControl2(humanoid,jointIds[i],p.TORQUE_CONTROL,targetPos, force=0.)	
		else:
