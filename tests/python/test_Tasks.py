...
class TestTasks(unittest.TestCase):
    def test_tasks(self):
        ...
        self.assertLess(np.linalg.norm(Kp - taskCOM.Kp, 2), tol)
        self.assertLess(np.linalg.norm(Kd - taskCOM.Kd, 2), tol)
        ...
        for i in range(0, max_it):
            ...
            self.assertLess(np.linalg.norm(Jpinv * const.matrix, 2) - 1.0, tol)
            v += dt * dv
            q = pin.integrate(model, q, dt * v)
            t += dt

            error = np.linalg.norm(taskCOM.position_error, 2)
            self.assertLess(error - error_past, 1e-4)
            error_past = error
            if error < 1e-8:
        ...
        self.assertLess(np.linalg.norm(Kp - task_joint.Kp, 2), tol)
        self.assertLess(np.linalg.norm(Kd - task_joint.Kd, 2), tol)
        ...
        self.assertLess(np.linalg.norm(Kp - task_se3.Kp, 2), tol)
        self.assertLess(np.linalg.norm(Kd - task_se3.Kd, 2), tol)
        ...
        self.assertLess(np.linalg.norm(Kp - taskAM.Kp, 2), tol)
        self.assertLess(np.linalg.norm(Kd - taskAM.Kd, 2), tol)
