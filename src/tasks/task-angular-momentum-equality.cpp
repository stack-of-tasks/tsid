...
namespace tsid
{
  namespace tasks
  {
    ...
    const Vector3 & TaskAMEquality::mask() const
    {
      return m_mask;
    }

    void TaskAMEquality::mask(const Vector3 & m)
    {
      //TODO: ggory15
      m_mask = m;
                                                    ...
                                                    const Data & data)
    {
      ...
      pinocchio::ccrba(m_robot.model(), data_ccrba, q, v);
      const Matrix6x & J_am = data_ccrba.Ag;
      m_L = J_am.bottomRows(3) * v;
      m_L_error = m_L - m_ref.pos;
      ...
      Data data_ccrba_next = data;
      pinocchio::ccrba(m_robot.model(), data_ccrba_next, q_next, v);
      const Matrix6x & J_am_next = data_ccrba_next.Ag;
      m_drift = (J_am_next.bottomRows(3) - J_am.bottomRows(3))* v / dt;
     
      for(unsigned int i=0; i<m_activeAxes.size(); i++){
        m_constraint.vector()(i) = m_dL_des(m_activeAxes(i)) - m_drift(m_activeAxes(i));
        m_constraint.matrix().row(i) = J_am.bottomRows(3).row(m_activeAxes(i));
      }
