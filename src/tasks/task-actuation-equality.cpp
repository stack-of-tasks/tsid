     ...
     m_constraint(name, robot.na(), robot.na())
   {
    ...
    void TaskActuationEquality::setNormalizationVector(ConstRefVector norm)
    {
      ...
      for(unsigned int i=0; i<m_activeAxes.size(); i++)
        m_constraint.matrix()(i,m_activeAxes(i)) = m_norm(m_activeAxes(i));
    }

    const Vector & TaskActuationEquality::getNormalizationVector() const
    {
      return m_norm;
    }
    ...
    {
      for(unsigned int i=0; i<m_activeAxes.size(); i++)
        m_constraint.vector()(i) = m_ref(m_activeAxes(i)) * m_norm(m_activeAxes(i));
