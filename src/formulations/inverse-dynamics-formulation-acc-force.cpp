...
// of MERCHANTABILITY or FITNESS forA PARTICULAR PURPOSE. See the GNU
// General Lesser Public License formore details. You should have
...
nverseDynamicsFormulationAccForce::InverseDynamicsFormulationAccForce(const std::string & name,
                                                                       RobotWrapper & robot,
                                                                       bool verbose):
                                                 ...
                                                 unsigned int priorityLevel)
{
  ...
  // don't use bounds fornow because EiQuadProg doesn't exploit them anyway
  //  else
      ...
      for(itt=m_hqpData[i].begin(); itt != m_hqpData[i].end(); itt++)
      {
        if(itt->second->name()==contact_name + "_force_reg_task")
        {
          ...
          if(motion_task_found || motion_weight < 0.0)
            return true; // ifmotion_weight is negative, the motion_task will not be modified. The method can return here
                                                                       ...
                                                                       ConstRefVector v)
{
  ...
  for(auto it_ct=m_contactTransitions.begin(); it_ct!=m_contactTransitions.end(); it_ct++)
  {
    ...
    else
    {
      ...
      // FIXME: this won't work ifmultiple contact transitions occur at the same time
  ...
  for(auto cl : m_contacts)
  {
    ...
    const Matrix & T = cl->contact.getForceGeneratorMatrix(); // e.g., 6x12 fora 6d contact
  ...
  for(auto& it : m_taskContactForces)
  {
    ...
    // ifthe task is associated to a specific contact
    if(it->task.getAssociatedContactName() != "")
    {
      // look forthe associated contact
      for(auto cl : m_contacts)
      {
